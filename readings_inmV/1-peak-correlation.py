...
for i, patient in enumerate(patients, 1):
    ...
    min_index = np.where(time_mit >= 0.5)[0][0]
    max_index = np.where(time_mit <= 1.3)[0][-1]
    ...
    if patient == '100':
        min_index = np.where(time_resampled >= 0.86)[0][0]
        max_index = np.where(time_resampled <= 1.66)[0][-1]
    elif patient == '102':
        min_index = np.where(time_resampled >= 0.9)[0][0]
        max_index = np.where(time_resampled <= 1.7)[0][-1]
    elif patient == '103':
        min_index = np.where(time_resampled >= 0.5)[0][0]
        max_index = np.where(time_resampled <= 1.3)[0][-1]
    elif patient == '104':
        min_index = np.where(time_resampled >= 1.8)[0][0]
        max_index = np.where(time_resampled <= 2.6)[0][-1]
