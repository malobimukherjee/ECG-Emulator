...
for i in range(len(arr2)):
    num[i, 0] = int((((float(arr2[i])) - min_value) * 255) / (max_value - min_value))
...
end_index = min(start_index + CHUNK_SIZE, len(arr2))
packet = num[start_index:end_index].flatten()
if len(packet) < CHUNK_SIZE:
    padding_length = CHUNK_SIZE - len(packet)
    packet = np.pad(packet, (0, padding_length), 'constant', constant_values=(0,))

for value in packet:
    byte = bytes([int(value)])
    ser.write(byte)
    time.sleep(0.001)
    #print(value)
...
j += 1
et1=datetime.now()
print("Time taken to sendone whole chunk", et1-st1)
