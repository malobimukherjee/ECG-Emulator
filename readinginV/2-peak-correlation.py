...
for i, patient in enumerate(patients, 1):
    ...
    if patient == '100':
        min_index = np.where(time_resampled >= 2.57)[0][0]
        max_index = np.where(time_resampled <= 4.07)[0][-1]
    elif patient == '102':
        min_index = np.where(time_resampled >= 0.75)[0][0]
        max_index = np.where(time_resampled <= 2.25)[0][-1]
    elif patient == '103':
        min_index = np.where(time_resampled >= 2)[0][0]
        max_index = np.where(time_resampled <= 3.5)[0][-1]
    elif patient == '104':
        min_index = np.where(time_resampled >= 1.7)[0][0]
        max_index = np.where(time_resampled <= 3.2)[0][-1]
